(function () {
  ...
  function renderVolunteer() {
    ...
    main.innerHTML = `<div class="volunteer-simple"><a class="back-link" href="#home">← Back to signup home</a>
      ${(volunteerStep === 0 || isClosed) ? `<section class="volunteer-event-card"><div class="volunteer-event-card__top"><span><i></i> Registration ${isClosed ? "closed" : "open"}</span><b>${Math.max(0, event.volunteerCapacity - state.volunteers.length)} spots left</b></div><div class="volunteer-event-card__body"><div class="volunteer-date"><small>DEC</small><strong>05</strong></div><div><p class="eyebrow">Featured event</p><h1>${esc(event.title)}</h1><p><strong>${esc(event.date)} · ${esc(event.time)}</strong></p><p>${esc(event.location)}</p>
  ...
  function organizerEventsV8() {
    ...
    html = html.replace(`<div class="event-context event-context--simple"><label>Event being edited<select><option>${esc(state.event.title)}</option><option>Christmas Shopping 2025 (closed record)</option></select></label></div>`, `<div class="event-context event-context--simple"><label>Active event being edited<select aria-label="Active event being edited"><option>${esc(state.event.title)}</option></select></label></div>`);
  ...
  function organizerBadgesV9() {
    const volunteerBadges = state.volunteers.filter((item) => item.currentEvent);
    const childBadges = state.applications.filter((item) => !item.archived).flatMap((household) => household.children.filter((child) => child.decision === "approved").map((child) => ({ child, household })));
    const actions = `<div class="heading-actions"><button class="button button--light" type="button" data-print-badges="volunteers">Download volunteer badges</button><button class="button button--green" type="button" data-print-badges="children">Download child badges</button></div>`;
    return `${heading("Event-day printing", "Print badges", "Create one volunteer badge file or one child badge file. Child badges are grouped efficiently on each page.", actions)}
      <div class="badge-settings panel"><div class="field"><label for="volunteer-badge-stock">Volunteer sticker sheet<select id="volunteer-badge-stock" data-badge-setting="volunteer"><option value="avery-8395">Avery 8395 · 3⅜ × 2⅓ in · 8 per sheet</option><option value="horizontal-35">Generic horizontal · 3½ × 2¼ in</option></select></label></div><div class="field"><label for="child-badge-size">Child badge holder insert<select id="child-badge-size" data-badge-setting="child"><option value="vertical-3x4">Vertical · 3 × 4 in · 4 per page</option><option value="vertical-225x35">Vertical · 2¼ × 3½ in · up to 9 per page</option></select></label></div><p>Each download button opens the browser's print screen. Choose <strong>Save as PDF</strong> to download the badge file, or select a printer to print immediately.</p></div>
      ...
      <div class="badge-section-heading"><div><h2 class="section-title">Child badges</h2><p>${childBadges.length} vertical cardstock inserts, arranged several to a page whenever space permits.</p></div></div><div class="badge-grid badge-grid--children">${childBadges.map(({ child, household }) => `<article class="badge-card badge-card--recipient" data-badge-id="${esc(child.id)}" data-badge-kind="children"><div class="badge-photo">${child.photoName ? "Child photo" : "Photo"}</div><img class="badge-logo" src="../assets/images/ccc-logo.png" alt=""><span>Campbell's Crew Cares</span><strong>${esc(child.name)}</strong><b>${esc(household.id)}</b></article>`).join("")}</div>
      <article class="badge-size-note"><h2>Current badge sizes</h2><p><strong>Volunteers:</strong> <span id="volunteer-size-summary">Avery 8395-compatible removable adhesive badges, 3⅜ × 2⅓ inches, horizontal, 8 per letter-size sheet.</span></p><p><strong>Children:</strong> <span id="child-size-summary">3 × 4 inch vertical cardstock inserts, arranged 4 per letter-size page.</span></p><p>Always print at <strong>100% / Actual Size</strong>; disable “Fit to page” so badge measurements remain accurate.</p></article>`;
  }
  ...
  function openCreateEvent() {
    ...
    dialogContent.querySelector("#create-event-form").addEventListener("submit", (event) => {
      ...
      state.event = fresh; state.activity.unshift({ text:`${fresh.title} was created as a new event draft.`, time:"Just now" }); saveState(); closeDialog(); toast("New event draft created. Signups remain closed until you open them."); renderOrganizer("events");
    });
  }
