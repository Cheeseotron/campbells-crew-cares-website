(function () {
  ...
  function organizerVolunteersV6(filter = "") {
    ...
    return `${heading("People & participation", "Volunteer directory", volunteerView === "all" ? "Every current and past volunteer remains searchable here." : `People currently registered for ${state.event.title}.`, `<button class="button button--light" type="button" data-export="volunteers">Download volunteers.csv</button>`)}<div class="table-tools"><label class="search-field"><span class="screen-reader-only">Search volunteers</span><input id="volunteer-search" type="search" value="${esc(filter)}" placeholder="Search name, email, phone, role, or event"></label><span>${rows.length} volunteers</span></div>${sortBar("volunteers",volunteerSort,[["name-az","First name"],["last-az","Last name"],["role","Role"],["no-shows","No-shows"]])}<div class="directory-list">${rows.map((item)=>{const history=item.history||[];const noShows=history.filter(x=>x.result==="No-show").length;const registered=Boolean(item.currentEvent);const removeAction=volunteerView==="current"&&registered?`<button class="button button--small button--danger" type="button" data-remove-volunteer-signup="${esc(item.id)}" data-volunteer-name="${esc(item.name)}">Remove from current signup</button>`:"";return `<details class="directory-card"><summary><span><strong>${esc(item.name)}</strong><small>${esc(item.email)} · ${esc(item.phone)}</small></span><span><b>${registered?esc(item.role):"Not currently signed up"}</b><small>${history.length} prior event${history.length===1?"":"s"}${noShows?` · ${noShows} no-show`:""}</small></span>${registered?statusPill("approved","Currently registered"):statusPill("closed","Directory volunteer")}</summary><div class="directory-card__body"><div><h3>Current registration</h3><p>${registered?`${esc(item.currentEvent)} · ${esc(item.role)} · ${esc(item.shift)}`:"Not registered for a current event."}</p><p><strong>Organizer note:</strong> ${esc(item.notes||"No notes")}</p>${removeAction}</div><div><h3>Event history</h3>${history.length?history.map((entry)=>`<p class="history-row"><span>${esc(entry.event)} · ${esc(entry.role)}</span><b class="${entry.result==="No-show"?"text-danger":""}">${esc(entry.result)}</b></p>`).join(""):`<p>No prior events recorded.</p>`}</div></div></details>`;}).join("")}</div>`;
  }
  ...
  function bindOrganizer(subroute) {
    ...
    document.querySelectorAll("[data-remove-volunteer-signup]").forEach((button) => button.addEventListener("click", async () => {
      const name = button.dataset.volunteerName || "this volunteer";
      if (!window.confirm(`Remove ${name} from the current event signup? Their permanent directory profile will be kept.`)) return;
      button.disabled = true; button.textContent = "Removing…";
      try {
        const response = await fetch(`/portal-api/organizer/volunteers/${encodeURIComponent(button.dataset.removeVolunteerSignup)}`, { method: "DELETE" });
        const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "The signup could not be removed.");
        await loadLiveVolunteers(); renderOrganizer("volunteer-signups"); toast(`${name} was removed from the current event signup.`);
      } catch (error) { toast(error.message || "The signup could not be removed."); button.disabled = false; button.textContent = "Remove from current signup"; }
    }));
