(function () {
  ...
  function organizerEventsV8() {
    ...
    html = html.replace(`<div class="event-context event-context--simple"><label>Event being edited<select><option>${esc(state.event.title)}</option><option>Christmas Shopping 2025 (closed record)</option></select></label></div>`, `<div class="event-context event-context--simple"><label>Active event being edited<select aria-label="Active event being edited" data-active-event>${activeEventOptions()}</select></label><button class="button button--green" type="button" data-create-event>+ Add Event</button></div>`);
